...
def test_predict_with_extreme_values(test_client, mock_env_api_key):
    ...
    mixed_values = {"features": [-0.5 if i % 2 == 0 else 0.5 for i in range(128)], "api_key": "test_key"}
...
def test_predict_with_malformed_features(test_client, mock_env_api_key):
    ...
    mixed_types = {"features": [0.1, "string", 1, True] + [0.1] * 124, "api_key": "test_key"}
    response = test_client.post("/predict", json=mixed_types)
    assert response.status_code == 422  # Validation error


# Test with different HTTP methods
def test_predict_with_different_http_methods(test_client, mock_env_api_key, sample_data):
