...
class UserService:
    ...
    def create_user(self, username: str, email: str, password: str, role: str = "user") -> models.User:
        """Create a new user"""
        # Check if user already exists
        existing_user = self.db.query(models.User).filter(
            (models.models.User.username == username) | (models.models.User.email == email)
        ).first()
    ...
    def authenticate_user(self, username: str, password: str) -> Optional[models.User]:
        ...
        user = self.db.query(models.User).filter(
            and_(models.models.User.username == username, models.models.User.is_active == True)
        ).first()
    ...
    def get_user_by_id(self, user_id: int) -> Optional[models.User]:
        ...
        return self.db.query(models.User).filter(
            and_(models.models.User.id == user_id, models.models.User.is_active == True)
        ).first()
    ...
    def get_users(self, skip: int = 0, limit: int = 100) -> List[models.User]:
        """Get list of users"""
        return self.db.query(models.User).filter(models.models.User.is_active == True).offset(skip).limit(limit).all()
    ...
    def validate_api_key(self, key: str) -> Optional[dict]:
        ...
        api_key = self.db.query(models.ApiKey).filter(
            and_(
    ...
    def revoke_api_key(self, key: str) -> bool:
        ...
        api_key = self.db.query(models.ApiKey).filter(models.ApiKey.key_hash == key_hash).first()
        if not api_key:
            return False
