...
class NotificationService:
    ...
    def get_notification_stats(self, user_id: int) -> Dict[str, int]:
        ...
        total = self.db.query(models.Notification).filter(
            models.Notification.user_id == user_id
        ).count()
        
        unread = self.db.query(models.Notification).filter(
            models.Notification.user_id == user_id,
            models.Notification.is_read == False
        ).count()
    ...
    def notify_model_training_complete(self, user_id: int, model_name: str, success: bool, metrics: Dict = None):
    ...
    def notify_dataset_uploaded(self, user_id: int, dataset_name: str, row_count: int):
    ...
    def notify_prediction_batch_complete(self, user_id: int, batch_size: int, success_count: int):
    ...
    def notify_api_key_created(self, user_id: int, key_name: str):
    ...
    def notify_account_activity(self, user_id: int, activity: str, ip_address: str = None):
    ...
    def send_admin_alert(self, title: str, message: str, level: str = "warning"):
