    ...
    async def _log_response(self, request: Request, response, process_time: float):
        ...
        try:
            ...
            if response.status_code >= 500:
                logger.error(f"Response: {request.method} {request.url.path} - {response.status_code}", extra=log_data)
            elif response.status_code >= 400:
                logger.warning(f"Response: {request.method} {request.url.path} - {response.status_code}", extra=log_data)
            else:
