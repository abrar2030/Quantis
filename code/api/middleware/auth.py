...
JWT_SECRET = os.getenv("JWT_SECRET", "your-secret-key-should-be-in-env-change-this-in-production")
JWT_ALGORITHM = "HS256"
...
async def validate_jwt_token(token: str = Depends(oauth2_scheme), db: Session = Depends(get_db)):
...
async def optional_auth(api_key: str = Security(API_KEY_HEADER, auto_error=False), db: Session = Depends(get_db)):
...
class RateLimiter:
    ...
    def __call__(self, user: dict = Depends(validate_api_key)):
        ...
        limit = self.requests_per_minute * 2 if user.get("role") == Roles.ADMIN else self.requests_per_minute
