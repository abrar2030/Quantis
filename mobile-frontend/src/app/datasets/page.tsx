        ...
        <CardContent className="p-6 space-y-6">
          ...
          {datasets.length === 0 ? (
            <div className="text-center py-8">
              ...
              <p className="text-gray-500">Start by uploading your first dataset!</p>
            </div>
          ) : (
            <ScrollArea className="h-[400px] w-full rounded-md border border-gray-700 bg-gray-900/50 p-3">
              <div className="space-y-4">
                {datasets.map((dataset) => (
                  <Card key={dataset.id} className="bg-gray-700/50 border-gray-600">
                    <CardContent className="p-4">
                      <div className="flex justify-between items-start mb-2">
                        <div>
                          <h3 className="text-xl font-semibold text-blue-300">{dataset.name}</h3>
                          <p className="text-sm text-gray-400">{dataset.description || 'No description provided.'}</p>
                        </div>
                      ...
                      <div className="grid grid-cols-2 gap-2 text-sm text-gray-400">
                        <p><strong>Rows:</strong> {dataset.row_count?.toLocaleString()}</p>
                        <p><strong>Columns:</strong> {dataset.columns?.length}</p>
                        <p><strong>Size:</strong> {formatFileSize(dataset.file_size)}</p>
                        <p><strong>Uploaded:</strong> {new Date(dataset.created_at).toLocaleDateString()}</p>
                      </div>
                      {dataset.metadata && (
                        <div className="mt-2">
                          {dataset.metadata.numeric_columns && dataset.metadata.numeric_columns.length > 0 && (
                            <p className="text-xs text-gray-500">Numeric: {dataset.metadata.numeric_columns.join(', ')}</p>
                          )}
                          {dataset.metadata.categorical_columns && dataset.metadata.categorical_columns.length > 0 && (
                            <p className="text-xs text-gray-500">Categorical: {dataset.metadata.categorical_columns.join(', ')}</p>
                          )}
                        </div>
                      )}
                    </CardContent>
