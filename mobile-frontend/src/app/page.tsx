...
import { Loader2, LogOut, BrainCircuit, History, Trash2, LineChart as LineChartIcon } from 'lucide-react' // Import icons
...
export default function Home() {
  ...
  const handlePredict = async () => {
    ...
    try {
      ...
      setPredictionHistory(prevHistory => [newHistoryEntry, ...prevHistory].slice(0, 10)); // Keep latest 10
      ...
      <Card className="w-full max-w-lg bg-gray-800/80 backdrop-blur-sm border border-gray-700 shadow-xl rounded-xl overflow-hidden">
        <CardHeader className="text-center p-6 bg-gray-800/50 border-b border-gray-700">
          <div className="flex justify-center items-center mb-3">
             ...
             <CardTitle className="text-3xl font-bold tracking-tight text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-purple-500">Quantis Predictor</CardTitle>
          </div>
          <CardDescription className="text-gray-400">
            Welcome back, <span className="font-medium text-gray-300">{userEmail || 'User'}</span>! Input features for prediction.
          </CardDescription>
        </CardHeader>
        <CardContent className="p-6 space-y-6">
          <div className="space-y-2">
            <Label htmlFor="features" className="text-sm font-medium text-gray-300">Input Features</Label>
            <Input
              id="features"
            ...
            />
            <p className="text-xs text-gray-500">Enter numerical features separated by commas.</p>
          </div>
          <Button 
          ...
          {error && (
            <Alert variant="destructive" className="w-full bg-red-900/30 border-red-700 text-red-300 rounded-lg">
              <AlertTitle className="font-semibold">Error</AlertTitle>
              <AlertDescription>{error}</AlertDescription>
            </Alert>
          )}
          {predictionResult && (
            <Alert variant="default" className="w-full bg-gray-700/50 border-gray-600 text-gray-300 rounded-lg">
               <AlertTitle className="font-semibold flex items-center mb-2">
                 <LineChartIcon className="h-5 w-5 mr-2 text-blue-400"/> Prediction Result
               </AlertTitle>
                    ...
                    <div className="h-[200px] w-full">
                      <ResponsiveContainer width="100%" height="100%">
                        ...
                        >
                          ...
                          <Line type="monotone" dataKey="value" name="Prediction" stroke="#60A5FA" strokeWidth={2} activeDot={{ r: 6 }} />
                        </LineChart>
                      </ResponsiveContainer>
                    </div>
                 ) : (
                   <p className="text-lg">Prediction: <span className="font-bold text-blue-300">{JSON.stringify(predictionResult.prediction)}</span></p>
                 )}
                 <p className="text-sm pt-1">Confidence: <span className="font-semibold text-green-400">{(predictionResult.confidence * 100).toFixed(2)}%</span></p>
               </AlertDescription>
            </Alert>
          )}
            ...
            <div className="mt-6 pt-6 border-t border-gray-700">
              <div className="flex justify-between items-center mb-3">
                <h3 className="text-xl font-semibold text-gray-300 flex items-center">
                  <History className="mr-2 h-5 w-5 text-purple-400" /> Prediction History
                </h3>
                <Button variant="ghost" size="sm" onClick={clearHistory} className="text-red-400 hover:text-red-300 hover:bg-red-900/30">
              ...
              <ScrollArea className="h-[200px] w-full rounded-md border border-gray-700 bg-gray-900/50 p-3">
                <div className="space-y-3">
                  {predictionHistory.map((entry) => (
                    <div key={entry.timestamp} className="text-sm p-2 rounded bg-gray-700/50">
                      <p className="text-xs text-gray-500 mb-1">{new Date(entry.timestamp).toLocaleString()}</p>
                      <p className="text-gray-400 truncate"><span className="font-medium text-gray-300">Input:</span> {entry.features}</p>
                      <p className="text-blue-300"><span className="font-medium text-gray-300">Pred:</span> {JSON.stringify(entry.prediction)} (<span className="font-medium text-gray-300">Conf:</span> {(entry.confidence * 100).toFixed(1)}%)</p>
                    </div>
                  ))}
                </div>
              </ScrollArea>
        ...
        <CardFooter className="flex flex-col items-center p-6 bg-gray-800/50 border-t border-gray-700">
           <Button variant="outline" onClick={handleLogout} className="w-full bg-transparent border-gray-600 hover:bg-gray-700/50 text-gray-300 hover:text-white rounded-lg flex items-center justify-center">
